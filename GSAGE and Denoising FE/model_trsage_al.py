...
class INTP_Model(nn.Module):
    def __init__(self, settings, device):
        ...
        self.gsage = GraphSAGE(in_channels=settings['embedding_dim'], hidden_channels=settings["conv_dim"], num_layers=2, out_channels=settings["conv_dim"]).to(self.device)
        ...
        for i in range(0, self.aux_task_num + 1):
            ...
            head = nn.Linear(settings["conv_dim"], 1)
    ...
    def forward(self, batch):
        q_tokens, known_lenths, auxil_lenths, input_series, answers, aux_answers = batch
        
        q_tokens = q_tokens.to(self.device)
        known_lenths = known_lenths.to(self.device)
        auxil_lenths = auxil_lenths.to(self.device)
        input_series = input_series.to(self.device)
        answers = answers.to(self.device)

        aux_answers = [item.to(self.device) for item in aux_answers]
        ...
        shared_output = self.gsage(x_l, edge_index, edge_weight)

        output = self.task_heads[0](shared_output)
        ...
        if self.aux_task_num >= 1:
            for i in range(1, self.aux_task_num + 1):
                aux_output = self.task_heads[i](shared_output)
        ...
        if len(self.task_heads) == 1:
            ...
            aux_target_head = aux_answers
    
        model_output = (output_head, mu_d_cut, std_d_cut, noise_d_cut)
        # model_output = output_head
        
        return model_output, target_head, aux_output_head, aux_target_head
