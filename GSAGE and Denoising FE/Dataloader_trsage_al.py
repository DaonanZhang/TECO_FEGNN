...
class IntpDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            df_auxil = df_filtered[df_filtered['op'].isin(self.env_op_dic.keys())].copy()
        ...
        aux_answers = [torch.zeros(1, dtype=torch.float) for _ in range(self.aux_task_num)]
        
        for op_index, aux_op in enumerate(self.aux_op_dic.keys()):
            aggregated_aux_op = aggregated_aux[aggregated_aux['op'] == aux_op]
            for features_df_index, row in target_row.iterrows():
                xi = row['Longitude']
                yi = row['Latitude']
                ...
                if (not matched_aux.empty):
                    assigned_value = torch.tensor([matched_aux['Result_norm'].values[0]], dtype=torch.float)
                    # print('------------------------')
                    # print(f'target row: {target_row}')
                    # print(f'matched_aux: {matched_aux}')
                    # print('------------------------')
                else:
                    # choose -1 as our Masked value
                    mask_value = torch.tensor([-float('inf')], dtype=torch.float)
                    assigned_value = mask_value

                aux_answers[op_index] = assigned_value
        ...
        return q_serie, known_serie, auxil_serie, answer, aux_answers
...
def collate_fn(examples):
    ...
    for i in range(0, task_num):
        sequence = pad_sequence([ex[4][i] for ex in examples], batch_first=True, padding_value=0.0)
        aux_y_b.append(sequence)
        
    return q_series, known_lenths, auxil_lenths, input_series, answers, aux_y_b
