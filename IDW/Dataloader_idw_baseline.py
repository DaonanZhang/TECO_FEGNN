...
class IntpDataset(Dataset):
    def __init__(self, settings, mask_distance, call_name):
        ...
        with open(self.origin_path + f"Folds_Info/norm_4_{settings['holdout']}.json", 'r') as f:
            self.dic_op_minmax = json.load(f)
        with open(self.origin_path + f"Folds_Info/divide_set_4_{settings['holdout']}.info", 'rb') as f:
    ...
    def __getitem__(self, idx):
        ...
        # print(f'target_row_value: {target_row["Result_norm"].values}')
        # # print(f'answer: {answers[0]}')
        # print(f'IDW predict: {features[0,0]}')

        # print(f' features.shape {features.shape} answers.shape {answers.shape}')
