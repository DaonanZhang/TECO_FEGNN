...
class IntpDataset(Dataset):
    def __init__(self, settings, mask_distance, call_name):
        ...
        self.env_op_dic = {'ta': 0, 'hur': 1, 'plev': 2, 'precip': 3, 'wsx': 4, 'wsy': 5, 'globalrad': 6}
    ...
    def __getitem__(self, idx):
        ...
        df_env = df_filtered[df_filtered['op'].isin(self.env_op_dic.keys())].copy()
        ...
        # _________________________________________AL mode_______________________________________________________

        # first norm then convert to torch 
        conditions = graph_candidates['op'] == self.dataset_info["tgt_op"]

        norm_value = graph_candidates['Thing'] / self.dataset_info["non_eu_col"]["Thing"][1]
        
        fallback_value = (self.dataset_info['lowest_rank'] - 1) / self.dataset_info["non_eu_col"]["Thing"][1]

        graph_candidates['Thing'] = np.where(conditions, norm_value, fallback_value)

        features = graph_candidates[['Result_norm', 'Thing']]
        ...
        aggregated_env = df_env.groupby(['Longitude', 'Latitude', 'op']).mean().reset_index()
        df_one_hot = pd.get_dummies(aggregated_env, columns=['op'])
        required_ops = list(self.env_op_dic.keys())
...
def collate_fn(examples):
    ...
    env_features = pad_sequence([ex[4] for ex in examples if len(ex[0]) > 2], batch_first=True, padding_value=0.0)
    auxil_lenths = torch.tensor([len(ex[4]) for ex in examples if len(ex[0]) > 2])
    
    return x_b, c_b, y_b, aux_y_b, input_lengths, env_features, auxil_lenths