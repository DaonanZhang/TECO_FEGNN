...
class PEGCN(nn.Module):
    ...
    def __init__(self, num_features_in=3, num_features_out=1, emb_hidden_dim=128, emb_dim=16, k=20, conv_dim=64,
        ...
        # -----------------Transformer Encoder-----------------
        self.transformer_Q = torch.nn.Parameter(torch.randn((1, 1, settings['d_model']), requires_grad=True, device=self.device))
    ...
    def forward(self, inputs, targets, aux_answers, coords, input_lengths, env_features, auxil_lenths, head_only):
        ...
        q_token_emb = self.transformer_Q.repeat(env_features.size(0), 1, 1)
        ...
        c_l, _ = padded_seq_to_vectors(coords, input_lengths)

        env_emb = Q_feature_emb.unsqueeze(1).expand(-1, pe.size(1), -1)
        env_emb = torch.cat([env_emb, pe], dim=-1)

        env_l, indexer = padded_seq_to_vectors(env_emb, input_lengths)
