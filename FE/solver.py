...
def training(settings, job_id):
    ...
    dataloader_tr = torch.utils.data.DataLoader(dataset_train, batch_size=settings['batch'], shuffle=True, collate_fn=dl.collate_fn, num_workers=4, prefetch_factor=32, drop_last=True)
    ...
    model = PEGCN(num_features_in=settings['num_features_in'], num_features_out=settings['num_features_out'],
                  ...
                  conv_dim=settings['conv_dim'], settings=settings).to(device)
    model = model.float()
        ...
        try:
            ...
            with torch.no_grad():
                for dataloader_ex in test_dataloaders:
                    ...
                    for x_b, c_b, y_b, input_lengths,env_features, auxil_lenths in dataloader_ex:
           ...
           # -----------------restore result-----------------
            ...
            x_b, c_b, y_b, input_lengths, env_features, auxil_lenths = batch

            x_b, c_b, y_b, input_lengths, env_features, auxil_lenths = x_b.to(device), c_b.to(device), y_b.to(device), input_lengths.to(device), env_features.to(device), auxil_lenths.to(device)
...
def evaluate(settings, job_id):
    ...
    for mask_distance in [10]:
        ...
        output_list = []
        target_list = []
        for x_b, c_b, y_b, input_lengths, env_features, auxil_lenths in dataloader_ev:
