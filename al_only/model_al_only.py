...
def padded_seq_to_vectors(padded_seq, logger):
    ...
    actual_lengths = actual_lengths.to(padded_seq.device)
...
class PEGCN(nn.Module):
    ...
    def forward(self, inputs, targets, aux_answers, coords, input_lengths, head_only):
        ...
        pe = self.spdec(pe)
        pe_l, indexer = padded_seq_to_vectors(pe, input_lengths)
