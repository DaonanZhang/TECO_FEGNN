...
import Dataloader_reg4_al_only as dl
from model_al_only import *
    ...
    while True:
        try:
            ...
            for dataloader_ex in test_dataloaders:
                ...
                for x_b, c_b, y_b, aux_y_b, input_lengths in dataloader_ex:
                    x_b, c_b, y_b, input_lengths = x_b.to(device), c_b.to(device), y_b.to(
                        device), input_lengths.to(device)

                    aux_y_b = [item.to(device) for item in aux_y_b]

                    outputs_b, targets_b, _, _ = model(x_b, y_b, aux_y_b, c_b, input_lengths, True)
            ...
            x_b, c_b, y_b, aux_y_b, input_lengths = batch
            ...
            outputs_b, targets_b, aux_outputs_b, aux_targets_b = model(x_b, y_b, aux_y_b, c_b, input_lengths, True)
            ...
            if (real_iter + 1) % settings['hyper_interval'] == 0:
                ...
                # ----------------------------meta_loss----------------------------
                meta_x_b, meta_c_b, meta_y_b, meta_aux_y_b, meta_input_lengths = aux_batch

                meta_x_b, meta_c_b, meta_y_b, meta_input_lengths = meta_x_b.to(device), meta_c_b.to(
                    device), meta_y_b.to(device), meta_input_lengths.to(device)
                ...
                meta_outputs_b, meta_targets_b, _, _ = model(inputs=meta_x_b, targets=meta_y_b, coords=meta_c_b,
                                                            ...
                                                            head_only=True, aux_answers=meta_aux_y_b)
                ...
                train_x_b, train_c_b, train_y_b, train_aux_y_b, train_input_lengths = train_batch2

                train_x_b, train_c_b, train_y_b, train_input_lengths = train_x_b.to(
                    device), train_c_b.to(device), train_y_b.to(device), train_input_lengths.to(
                    device)
                ...
                loss_list = [primary_loss] + aux_batch_loss_list
...
def evaluate(settings, job_id):
    ...
    for mask_distance in [10]:
        ...
        output_list = []
        target_list = []
        for x_b, c_b, y_b, aux_y_b, input_lengths in dataloader_ev:
